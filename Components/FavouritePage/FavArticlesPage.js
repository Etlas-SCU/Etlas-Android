...
export default function FavArticlesPage({ navigation }) {

    // get the article list
    const ArticlesList = Backend.getFavArticles();

    // mapping the cards
    const Articles = ArticlesList.map((Article, index) => { return <FavArticleCard key={index} Article={Article} navigation={navigation} /> });
